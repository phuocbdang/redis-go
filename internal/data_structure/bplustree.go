/*
Balanced Tree: leaf nodes have same height
Each node has M-1 keys, M children
...
func (i *Item) CompareTo(other *Item) int {
	...
	// Score are equal, use the member string for tie-breaking
...
// Node of B+ Tree
type Node struct {
	Items    []*Item // A list of key-value pairs, maximum M-1 items
	Children []*Node // Pointers to child nodes, maximum M items
...
func (t *BPlusTree) splitLeaf(node *Node) {
	...
	newLeaf := &Node{
		Items:  node.Items[medianIndex:],
		IsLeaf: true,
		Parent: node.Parent,
		Next:   node.Next,
	}
	// Keep only the first half of the items in the original node
	...
	parent.Items = append(
		...
		append([]*Item{promotedItem}, parent.Items[childIndex:]...)...,
	)
	parent.Children = append(
		parent.Children[:childIndex+1],
		append([]*Node{newLeaf}, parent.Children[childIndex+1:]...)...,
	)
...
func (t *BPlusTree) splitInternal(node *Node) {
	...
	// Update the parent pointers for the new children
	...
	parent.Children = append(
		parent.Children[:childIndex+1],
		append([]*Node{newInternal}, parent.Children[childIndex+1:]...)...,
	)
