...
/* Push adds a new item to the pool, maintains the lastAccessTime accenting
 * order (old items are on the left)
 * If pool size > EpollMaxSize, removes the newest item */
func (p *EvictionPool) Push(key string, lastAccessTime uint32) {
	...
	/* Note: In Redis implementation, it does not explicitly check if a
	 * key is already in the eviction poll before attempting to insert it.
	 * This could lead to a key being in the pool twice if it's sampled
	 * and inserted a second time. However, since the eviction poll is
	 * very small (EpollMaxSize is 16) and the random sampling is just a
	 * small fraction of the total keys, the probability of this happening
	 * is extremely low
